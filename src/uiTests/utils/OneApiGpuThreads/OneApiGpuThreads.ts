...
import { ExecuteInOneApiDebugPaneFrame, GetDebugPane, GetStringBetweenStrings, Retry, SetInputText, Wait } from "../CommonFunctions";
        ...
        return await ExecuteInOneApiDebugPaneFrame(async(driver) => {
            ...
            for (const row of gpuThreadsRows) {
                ...
                for (const data of rowData) {
                    let cellGroups: WebElement[] | undefined = undefined;

                    try { cellGroups = await data.findElements(By.className("cell-group")); }
                    catch { /* empty */ }
